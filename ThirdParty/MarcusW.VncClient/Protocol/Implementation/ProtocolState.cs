...
namespace MarcusW.VncClient.Protocol.Implementation
{
    ...
    public class ProtocolState : IRfbProtocolState
    {
        ...
        private readonly StateValue<string?> _lastClientClipboardTextValue = new StateValue<string?>(null);
        ...
        public bool ServerSupportsExtendedClipboard
        {
            ...
            set
            {
                _serverSupportsExtendedClipboardValue.Value = value;
                _context.ConnectionDetails.SetServerSupportsExtendedClipboard(value);
            }
        }
        ...
        /// <summary>
        /// Gets the last client clipboard text sent through ClientCutText.
        /// </summary>
        public string? LastClientClipboardText => _lastClientClipboardTextValue.Value;

        /// <summary>
        /// Stores the last client clipboard text so server-side extended clipboard requests can be answered.
        /// </summary>
        /// <param name="text">The clipboard text.</param>
        public void SetLastClientClipboardText(string? text)
        {
            _lastClientClipboardTextValue.Value = text;
        }

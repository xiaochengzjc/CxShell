...
public partial class VncWindow : AtomUI.Desktop.Controls.Window
{
    ...
    {
        InitializeComponent();
        Opened += (_, _) => FramebufferImage?.Focus();
        Closed += (_, _) =>
        {
    ...
    private void SendPointer(PointerEventArgs e, byte mask)
    {
        ...
        var (x, y) = MapPointer(e.GetPosition(FramebufferImage), FramebufferImage.Bounds.Size, vm.RemoteWidth, vm.RemoteHeight);
        vm.SendPointer(mask, x, y);
    }

    private static (int X, int Y) MapPointer(Point point, Size imageSize, int remoteWidth, int remoteHeight)
    {
        if (remoteWidth <= 0 || remoteHeight <= 0 || imageSize.Width <= 0 || imageSize.Height <= 0)
            return (0, 0);

        var scale = Math.Min(imageSize.Width / remoteWidth, imageSize.Height / remoteHeight);
